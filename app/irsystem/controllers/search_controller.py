...
def _get_cos_sim_desc(book_ids: list) -> list:
	...
	result = sorted(cos_sim_sum, key=cos_sim_sum.get, reverse=True)[:MAX_RECOMMEND]
	return result
...
def _get_recommended_books_detail(recommended_book_ids: list) -> list:
	recommended_books = []
...
def search():
	...
	if request.method == "POST":
		...
		if len(book_ids) >= 1 and book_ids[0] != "":
			recommended_book_ids = _get_cos_sim_desc(book_ids)
			data = _get_recommended_books_detail(recommended_book_ids)
	else:
		data = []
